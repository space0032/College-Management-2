...
public class HostelController extends BaseController implements HttpHandler {
    ...
    public void handle(HttpExchange t) throws IOException {
        ...
        try {
            if (path.matches(".*/hostels/allocations/\\d+")) {
                if ("DELETE".equals(method)) handleVacate(t, path);
                else sendResponse(t, 405, errorJson("Method not allowed"));
            } else if (path.matches(".*/hostels/allocations.*")) {
                if ("GET".equals(method)) handleGetAllocations(t);
                else if ("POST".equals(method)) handleAllocate(t);
                else sendResponse(t, 405, errorJson("Method not allowed"));
            } else if (path.matches(".*/hostels/rooms.*")) {
                if ("GET".equals(method)) handleGetRooms(t);
                else if ("POST".equals(method)) handleAddRoom(t);
                else sendResponse(t, 405, errorJson("Method not allowed"));
            } else {
                if ("GET".equals(method)) handleGetHostels(t);
                else if ("POST".equals(method)) handleAddHostel(t);
                else sendResponse(t, 405, errorJson("Method not allowed"));
            }
        } catch (Exception e) {
    ...
    private void handleAddHostel(HttpExchange t) throws IOException {
        ...
        if (ok) sendResponse(t, 201, JsonHelper.toJson(hostel));
        else sendResponse(t, 400, errorJson("Failed to add hostel"));
    }
    ...
    private void handleAddRoom(HttpExchange t) throws IOException {
        ...
        if (ok) sendResponse(t, 201, JsonHelper.toJson(room));
        else sendResponse(t, 400, errorJson("Failed to add room"));
    }
    ...
    private void handleAllocate(HttpExchange t) throws IOException {
        ...
        if (ok) sendResponse(t, 201, JsonHelper.toJson(allocation));
        else sendResponse(t, 400, errorJson("Failed to allocate room"));
    }

    private void handleVacate(HttpExchange t, String path) throws IOException {
        int id = extractId(path);
        boolean ok = hostelDAO.vacateRoom(id);
        if (ok) sendResponse(t, 200, "{\"status\":\"Vacated\"}");
        else sendResponse(t, 400, errorJson("Failed to vacate room"));
    }
