...
public class AttendanceController extends BaseController implements HttpHandler {
    ...
    public void handle(HttpExchange t) throws IOException {
        ...
        try {
            if (path.matches(".*/attendance/student/\\d+")) {
                if ("GET".equals(method)) handleGetByStudent(t, path);
                else sendResponse(t, 405, errorJson("Method not allowed"));
            } else if (path.matches(".*/attendance/bulk")) {
                if ("POST".equals(method)) handleBulk(t);
                else sendResponse(t, 405, errorJson("Method not allowed"));
            } else {
                if ("GET".equals(method)) handleGetByCourseAndDate(t);
    ...
    private void handleGetByCourseAndDate(HttpExchange t) throws IOException {
        ...
        if (query != null) {
            for (String param : query.split("&")) {
                ...
                if (kv.length == 2) {
                    if ("courseId".equals(kv[0])) courseId = Integer.parseInt(kv[1]);
                    else if ("date".equals(kv[0])) {
                        try { date = new SimpleDateFormat("yyyy-MM-dd").parse(kv[1]); } catch (Exception ignored) {}
                    }
                }
            }
        }
    ...
    private void handlePost(HttpExchange t) throws IOException {
        ...
        if (ok) sendResponse(t, 201, "{\"status\":\"Attendance marked\"}");
        else sendResponse(t, 400, errorJson("Failed to mark attendance"));
    }
