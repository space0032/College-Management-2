...
public class DashboardView {
    ...
    private HBox createTopBar() {
        ...
        if (logoPath != null && !logoPath.isEmpty()) {
            ...
            new Thread(() -> {
                try {
                    java.io.File logoFile = storageService.getFile(fLogoPath);
                    if (logoFile != null && logoFile.exists()) {
                        String tempLink = logoFile.toURI().toString();
                        javafx.application.Platform.runLater(() -> {
                            logoView.setImage(new Image(tempLink, true));
                        });
                    }
                } catch (Exception e) {
                    com.college.utils.Logger.error("Failed to load logo in dashboard", e);
                }
            }).start();
        } else {
